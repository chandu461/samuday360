...
function guidmobileinsert()
{
    ...
    var db =window.sqlitePlugin.openDatabase({ name: 'samudayguid.db', location: 'default', androidDatabaseProvider: 'system' });
...
function maprendersearch(tx)
{
	...
	tx.executeSql('SELECT name, geom FROM guideblockboundaries', [], function(tx, result)
	{
		...
		if(result.rows.length>0)
		{
			...
			{   
				featureGeom = format.readFeature(result.rows.item(i).geom,{ dataProjection: 'EPSG:4326', featureProjection: 'EPSG:3857'});
				featureGeom.setStyle(
					...
					({
						...
						text:new ol.style.Text({text:result.rows.item(i).name})
					})
				);
	...
	tx.executeSql('SELECT name, geom FROM guidevillageboundaries', [], function(tx, result){
		...
		if(result.rows.length>0)
		{
			...
			{   
				...
				featureGeom.setProperties({'name':result.rows.item(i).name, 'geom':result.rows.item(i).geom});
	...
	tx.executeSql('SELECT resondantcode, respondantname, hohname, relationwithhoh, dateofbirth, gender, idtype, idnumber, buildingcode, housecode, blockname, gpname, village, occupation, buildingtype, geom FROM guidesurvey_bene', [], function(tx, result){
		...
		if(result.rows.length>0)
		{
			for (var i=0;i<result.rows.length;i++)
			{  
				blitems.push(result.rows.item(i));
				if(result.rows.item(i).geom!="null" && result.rows.item(i).geom!=null)
				{
					featureGeom = format.readFeature(result.rows.item(i).geom,{ dataProjection: 'EPSG:4326', featureProjection: 'EPSG:3857'});
					featureGeom.setStyle(survey_benstyle);
					featureGeom.setProperties({'respondentcode':result.rows.item(i).resondantcode, 'index':i});
	...
	tx.executeSql('SELECT DISTINCT buildingtype FROM guidesurvey_bene', [], function(tx, result){
		...
		{
			sbloop=i+1;
			elemfiltergp.options[sbloop]=new Option(result.rows.item(i).buildingtype,result.rows.item(i).buildingtype);
		}
	}, errorCB);
}
...
function searchsql(tx)
{
	tx.executeSql(guidequerystring, [], function(tx, result){
		...
		if(result.rows.length>0)
		{
			for (var i=0;i<result.rows.length;i++)
			{   
				if(result.rows.item(i).geom!="null" && WKTpoly[i].geom!=null)
				{
					featureGeom = format.readFeature(result.rows.item(i).geom,{ dataProjection: 'EPSG:4326', featureProjection: 'EPSG:3857'});
